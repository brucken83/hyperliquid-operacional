...
CONFIG = {"coins": ["BTC", "ETH"], "rsi2_threshold": 7, "pullback_lookback_bars": 5}
...
def candle_snapshot(coin, interval, limit=120):
    ...
    interval_ms = {"15m": 900_000, "4h": 14_400_000}[interval]
...
def send_telegram(text):
    ...
    requests.post(f"https://api.telegram.org/bot{TELEGRAM_BOT_TOKEN}/sendMessage",
                  json={"chat_id": TELEGRAM_CHAT_ID, "text": text}, timeout=20).raise_for_status()
...
def main():
    ...
    for coin in CONFIG["coins"]:
        ...
        if long_signal:
            ...
            sig = {"coin": coin, "side": "LONG", "entry": float(last15["close"]), "stop": stop,
                   "target1": float(last15["close"]) + (float(last15["close"]) - stop),
                   "target2": float(last15["close"]) + 2 * (float(last15["close"]) - stop),
                   "funding_hr_pct": funding, "time": str(last15["close_time"])}
            signals.append(sig)
            send_telegram(f"⚡ {coin} LONG\nEntrada: {sig['entry']:.2f}\nStop: {sig['stop']:.2f}\nT1: {sig['target1']:.2f}\nT2: {sig['target2']:.2f}\nFunding: {funding:.4f}%/h")

        if short_signal:
            stop = float(df15.iloc[-5:]["high"].max())
            sig = {"coin": coin, "side": "SHORT", "entry": float(last15["close"]), "stop": stop,
                   "target1": float(last15["close"]) - (stop - float(last15["close"])),
                   "target2": float(last15["close"]) - 2 * (stop - float(last15["close"])),
                   "funding_hr_pct": funding, "time": str(last15["close_time"])}
            signals.append(sig)
            send_telegram(f"⚡ {coin} SHORT\nEntrada: {sig['entry']:.2f}\nStop: {sig['stop']:.2f}\nT1: {sig['target1']:.2f}\nT2: {sig['target2']:.2f}\nFunding: {funding:.4f}%/h")

    (WEB_DATA / "latest_status.json").write_text(json.dumps(rows, indent=2), encoding="utf-8")
    (WEB_DATA / "signals.json").write_text(json.dumps(signals, indent=2), encoding="utf-8")
    ...
    (WEB_DATA / "paper_trades.json").write_text(json.dumps(paper, indent=2), encoding="utf-8")
